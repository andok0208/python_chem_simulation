...
nparticle = 3
xeq = 1.0
x =  np.array([-xeq, 0.0, xeq])       # 平衡位置
x += np.array([ 0.0, 0.0, 0.2])       # 変位
v = np.zeros(nparticle)               # 初期速度
m = np.array([4.0, 3.0, 4.0])         # 質量
k = np.zeros((nparticle, nparticle))  # 力の定数初期化
k[0,1], k[1,2] = 1.0, 1.0             # 力の定数代入
tmax, h = 500.0, 0.001
